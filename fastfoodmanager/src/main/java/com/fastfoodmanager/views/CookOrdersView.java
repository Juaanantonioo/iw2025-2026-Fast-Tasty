...
public class CookOrdersView extends VerticalLayout {
    ...
    private String formatItemsPreview(List<OrderItem> items) {
        ...
        return items.stream()
                .limit(2)
                .map(i -> {
                    if (i.isProduct()) return i.getProductName() + " x" + i.getQuantity();
                    if (i.isMenu()) return i.getMenuName() + " (Menú) x" + i.getQuantity();
                    return "Producto x" + i.getQuantity();
                })
                .reduce((a, b) -> a + ", " + b)
                .orElse("-")
    ...
    private void showOrderDetails(Order order) {
        ...
        if (order.getItems() != null && !order.getItems().isEmpty()) {
            ...
            for (OrderItem item : order.getItems()) {
                ...
                String nameText =
                        item.isProduct() ? item.getProductName() :
                                item.isMenu() ? item.getMenuName() + " (Menú)" :
                                        "Producto";
                ...
                // INGREDIENTES SOLO PARA PRODUCTOS
                if (item.isProduct() &&
